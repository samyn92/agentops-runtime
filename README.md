...
| Method | Path | Description |
|--------|------|-------------|
| `GET` | `/working-memory` | Get serialized sliding window messages. |
| `DELETE` | `/working-memory` | Clear all messages and reset turn counter. |
| `PATCH` | `/config/window-size` | Set window size: `{"size": N}` (min 2). |
...
| `GET` | `/status` | Runtime status: model, steps, busy, messages, window size, turns, memory enabled. |
